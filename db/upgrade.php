...
function xmldb_yesno_upgrade($oldversion) {
    ...
    if ($oldversion < 2026030705) {
        // Remove duplicate attempts, keeping only the most recent one per user/activity.
        $sql = "SELECT userid, yesnoid, MAX(id) as maxid
                FROM {yesno_attempts}
                GROUP BY userid, yesnoid
                HAVING COUNT(*) > 1";
        $duplicates = $DB->get_recordset_sql($sql);

        foreach ($duplicates as $dup) {
            // Delete all attempts except the one with the highest id (most recent).
            $DB->delete_records_select(
                'yesno_attempts',
                'userid = ? AND yesnoid = ? AND id != ?',
                [$dup->userid, $dup->yesnoid, $dup->maxid]
            );
        }
        $duplicates->close();

        // Make user_instance index unique.
        $table = new xmldb_table('yesno_attempts');
        $index = new xmldb_index('user_instance', XMLDB_INDEX_NOTUNIQUE, ['userid', 'yesnoid']);

        if ($dbman->index_exists($table, $index)) {
            $dbman->drop_index($table, $index);
        }

        $index = new xmldb_index('user_instance', XMLDB_INDEX_UNIQUE, ['userid', 'yesnoid']);
        if (!$dbman->index_exists($table, $index)) {
            $dbman->add_index($table, $index);
        }

        // Yesno savepoint reached.
        upgrade_mod_savepoint(true, 2026030705, 'yesno');
    }

    if ($oldversion < 2026030706) {
        // Convert unique user_instance index to non-unique to allow multiple attempts per user.
        $table = new xmldb_table('yesno_attempts');
        $index = new xmldb_index('user_instance', XMLDB_INDEX_UNIQUE, ['userid', 'yesnoid']);
